...
def run_docs(model=None, provider=None):
    """
    Generates documentation through the reusable core.
    """
    ...
    try:
        ...
        summaries = summarize_files(
            Path.cwd(), files, llm_config=llm_config, metadata=llm_metadata,
            progress=lambda stage, done, total, path: console.print(f"{done}/{total}: {path}"),
        )
